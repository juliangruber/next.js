...
describe('static App Shell prefetch attempt', () => {
  ...
  it('speculative: falls back to a runtime prefetch when the static attempt is insufficient', async () => {
    ...
    // prefetch={true}, so the static attempt and its runtime fallback also
    // happen for the page segment during the SPECULATIVE phase — and unlike
    // the divergent-cookies fixture, the Speculative runtime prefetch
    // resolves the cookies() read, so the session content itself arrives in
    // the runtime fallback response.
    //
    // Reset the fixture to its clean generation and poll until the server
    ...
    await act(async () => {
      ...
      // below, the runtime prefetch of this segment resolves
