  ...
  while (node !== null) {
    ...
    switch (nodeEntry.status) {
      ...
      case EntryStatus.Fulfilled: {
        ...
        // strategy tier: its remaining holes are dynamic — no prefetch can
        // fill them; they belong to the navigation-time dynamic request —
        // so an upgrade would only re-fetch content the entry already has.
        //
        // The signal only speaks for the variant the entry's content was
        // fetched for, which is what its recorded strategy describes (see
        // SegmentCacheEntryShared['fetchStrategy']). A shell-tier entry is
        // the App Shell variant: a concrete read finding it is reading a
        // DIFFERENT variant — a more specific fetch can still return the
        // param-dependent content the shell omits — so a shell-scoped
        // signal never suppresses the upgrade. (This is what keeps eager
        // segments prefetching per-link content on top of a cached shell.
        // A shell entry whose content was actually the full response is
        // recorded at the full tier at write time, so it doesn't hit this
        // carve-out — the ordinary strategy-tier check below stops its
        // re-fetch instead.)
