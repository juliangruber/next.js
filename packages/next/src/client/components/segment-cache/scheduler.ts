  ...
  while (node !== null) {
    ...
    switch (nodeEntry.status) {
      ...
      case EntryStatus.Fulfilled: {
        ...
        // it's partial and less specific: its remaining holes are dynamic —
        // no prefetch can fill them; they belong to the navigation-time
        // dynamic request — so an upgrade would only re-fetch content the
        // entry already has.
        //
        ...
        // signal never suppresses the upgrade. (A shell entry whose content
        // was actually the full response is recorded at the full tier at
        // write time, so it doesn't hit this carve-out — the ordinary
        // strategy-tier check below stops its re-fetch instead.)
